let GM=localStorage.getItem('fitlog_model')||'gemini-3.1-flash-lite';
        let API_URL='https://generativelanguage.googleapis.com/v1beta/models/'+GM+':generateContent';
  ...
  farmers_walk:BASE+'Farmers-Walk.gif',
        ...
        function openSettings(){$('settings-modal').style.display='flex';$('settings-api-key').value=S.key;if($('settings-rapidapi-key'))$('settings-rapidapi-key').value=S.rapidapi_key||'';const ms=$('settings-model');if(ms)ms.value=localStorage.getItem('fitlog_model')||'gemini-3.1-flash-lite';updateKeyStatus()}function closeSettings(){$('settings-modal').style.display='none'}
        function saveSettingsKey(){const k=$('settings-api-key')?.value?.trim();const rk=$('settings-rapidapi-key')?.value?.trim();if(k){S.key=k;localStorage.setItem('fitlog_key',k);}if(rk!==undefined){S.rapidapi_key=rk;localStorage.setItem('fitlog_rapidapi_key',rk);}const m=$('settings-model')?.value;if(m){localStorage.setItem('fitlog_model',m);GM=m;API_URL='https://generativelanguage.googleapis.com/v1beta/models/'+GM+':generateContent';}closeSettings();toast('✅ Saved!')}
